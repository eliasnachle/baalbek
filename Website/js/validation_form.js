...
// console.log(number);
// if(number <= 1){
//     frases.innerHTML += `A icônica introdução da música "O Ciclo da Vida" de "O Rei Leão", traduzida, diz "Lá vem um leão, ah sim, é um leão".`;
// } else if (number <=2){
//     frases.innerHTML += `O nome do protagonista da série “Naruto” significa redemoniho e é também o nome de uma cidade em Tokushima Prefecture of Japan, denominada Naruto.`;
// } else if(number <= 3){
//     frases.innerHTML += `O próprio Godzilla só foi visto por cerca de 8 minutos no filme de 2014 “Godzilla”.`;
// }else if (number <=4){
//     frases.innerHTML += `Pocahontas é a única princesa da Disney que possui uma tatuagem.`;
// } else if (number <= 5){
//     frases.innerHTML += `Nas gravações de "O Exorcista", o diretor William Friedkin pedia para algumas pessoas da equipe darem tiros de mentira para assustar o elenco.`;
// } else if (number <= 6){
//     frases.innerHTML += `O filme "Pânico" foi inspirado em fatos reais que ocorreram em Gainesville, Flórida (EUA), no início dos anos 90.`;
// } else if (number <= 7){
//     frases.innerHTML += `O especial de comédia de Eddie Murphy "Delirious" (1983) inspirou o filme "Corra".`;
// }
// }
...
function validate_date(){
    ...
    // Validação
    if (idade < 18){    
        msg_validate_date.innerHTML = 'Você precisa ser maior de idade'
    } else {
